...
class S3BucketConnector():
   ...
   def __init__(self, access_key: str, secret_key: str, endpoint_url: str, bucket: str):
      ...
      self.logger = Logger_Handler(logger_name="app_logger", log_file = 's3_connector.log', level=logging.INFO).get_logger()
   ...
   def read_csv_to_df(self, key:str, encoding:str = 'utf-8', sep:str = ','):
       ...
       self.logger.info(f'Reading file {self.endpoint_url}/{str(self._bucket.name)}/{key}')
   ...
   def write_df_to_s3(self, data_frame: pd.DataFrame, key:str, file_format: str):
       ...
       Args:
           data_frame (pd.DataFrame): Pandas DataFrame that should be written
           key (str): target key of the saved file
           file_format (str): format of the saved file
       """
       ...
       if file_format == S3FileTypes.PARQUET.value:
          ...
          return self.__put_object(out_buffer,key)
       self.logger.info('The file format %s is not supported to be written to s3!', file_format)
   ...
   def __put_object(self, out_buffer: Union[StringIO,BytesIO], key: str):
       ...
       try:
            self.logger.info('Writing file to %s/%s/%s', self.endpoint_url, self._bucket.name, key)
            self._bucket.put_object(Body=out_buffer.getvalue(), Key=key)
            self.logger.info('Archivo subido correctamente a S3')
            return True
       except Exception as e:
            self.logger.error('Fallo al subir archivo a S3: %s', str(e))
            return False
